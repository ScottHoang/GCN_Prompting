...
class PretrainLearner(Learner):
    ...
    def task_test_contrastive(self):
        ...
        logits, labels = self.info_nce_loss(embs_valid, data.val_pos_edge_index[0], data.val_pos_edge_index[1], data.val_neg_edge_index[0],
                                            data.val_neg_edge_index[1], self.temp)
        loss_valid = self.loss_fn(logits, labels)
        logits, labels = self.info_nce_loss(embs_test, data.test_pos_edge_index[0], data.test_pos_edge_index[1], data.test_neg_edge_index[0],
    ...
    def task_test_edges(self):
        ...
        val_pred_pos.append(torch.sigmoid(
            self.model.forward_edge(data.x, data.val_pos_edge_index, data.val_pos_edge_index[0], data.val_pos_edge_index[1])))
        val_pred_neg.append(torch.sigmoid(
            self.model.forward_edge(data.x, data.val_pos_edge_index, data.val_neg_edge_index[0], data.val_neg_edge_index[1])))
        test_pred_pos.append(torch.sigmoid(
            self.model.forward_edge(data.x, data.test_pos_edge_index, data.test_pos_edge_index[0], data.test_pos_edge_index[1])))
        test_pred_neg.append(torch.sigmoid(
            self.model.forward_edge(data.x, data.test_pos_edge_index, data.test_neg_edge_index[0], data.test_neg_edge_index[1])))
